(() => {
  ...
  const state = { year: null, category: null, questions: [], answers: {}, reasons: {}, optionReasons: {}, orders: {}, results: {}, checked: {}, aiChats: {}, startedAt: 0, timerId: null, submitted: false, replayMode: false, historyRecordId: null };
  ...
  function isOrderingQuestion(question) {
    return state.category === "语法" && Number(question.groupNumber) === 6 && (question.options || []).length === 4;
  }
  ...
  function questionMarkup(question, index, seenPassages) {
    ...
    const showPassage = question.passage && !seenPassages.has(passageKey);
    ...
    return `<article class="question-card" id="question-${question.id}" data-id="${question.id}">
      ...
      <div class="question-meta">
        ...
        <span class="question-source">大题 ${escapeHtml(question.groupNumber)}${sourceLabel}</span>
      </div>
  ...
  function startExam(savedDraft = null) {
    closeAiDrawer();
    ...
    $("#exam-title").textContent = `${state.year} · ${state.category}`;
    $("#back-button").textContent = "← 返回选择";
  ...
  function renderHistory() {
    ...
    if (yearFilter) {
      const years = DATA.years.filter((year) => history.some((record) => record.year === year));
    ...
    $("#history-list").innerHTML = filtered.map((record) => `
      <article class="history-row">
        <div><strong>${escapeHtml(record.year)} · ${escapeHtml(record.category)}</strong><p class="muted">${formatDate(record.submittedAt)}</p></div>
  ...
  function loadHistoryIntoExam(id, { restoring = false } = {}) {
    ...
    $("#setup-panel").hidden = true;
    $("#exam-panel").hidden = false;
    $("#exam-title").textContent = `历史回放 · ${record.year} · ${record.category}`;
  ...
  function restoreUiState() {
    ...
    const view = ["practice", "history", "ai-config"].includes(saved.view) ? saved.view : "practice";
    if (view === "practice" && saved.examActive) {
